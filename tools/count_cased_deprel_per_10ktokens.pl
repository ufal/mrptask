...
print("  <tr><th>Treebank</th><th>Tokens</th><th>Case-augmented relations</th><th>Cased per 10k tokens</th></tr>\n");
while(<SRC>)
{
    ...
    elsif(m/Deprel with case:\s*(\d+)/)
    {
        $ncased = $1;
        # Number of empty nodes always appears after number of tokens. So we can report now.
        my $ratio = sprintf("%d", $ncased/($ntokens/10000)+0.5);
        print("  <tr><td>$current_treebank</td><td>$ntokens</td><td>$ncased</td><td>$ratio</td></tr>\n");
    }
}
print("</table>\n");
close(SRC);
