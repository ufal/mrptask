...
print("  <tr><th>Treebank</th><th>Tokens</th><th><tt>ref</tt> relations</th><th>Refs per 10k tokens</th></tr>\n");
while(<SRC>)
{
    ...
    elsif(m/Relative clause:\s*(\d+)/)
    {
        $nref = $1;
        # Number of empty nodes always appears after number of tokens. So we can report now.
        my $ratio = sprintf("%d", $nref/($ntokens/10000)+0.5);
        #print("$current_treebank\t$ntokens\t$nempty\t$ratio\n");
        print("  <tr><td>$current_treebank</td><td>$ntokens</td><td>$nref</td><td>$ratio</td></tr>\n");
    }
}
print("</table>\n");
close(SRC);
